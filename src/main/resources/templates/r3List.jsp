...
<div class="container-fluid">
    <button class="btn btn-secondary disabled">文档资源管理</button>
    <div class=" py-2 d-flex">
        <form class="d-flex w-50" method="post" action="${pageContext.request.contextPath}/res/doc">
        ...
        <c:forEach items="${requestScope.page.dataList}" var="res">
            <tr>
                ...
                <td>
                    ...
                    <a class="btn btn-outline-secondary" href="${pageContext.request.contextPath}/res/doc?act=comment&id=${res.id}">评论</a>
                </td>
            </tr>
        </c:forEach>
...
<nav aria-label="Page navigation example">
    <ul class="pagination justify-content-center">
        <c:if test="${requestScope.page.currentPage>1}">
            <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/res/doc?resname=${requestScope.resname}&status=${requestScope.status}">首页</a></li>
            <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/res/doc?cur=${requestScope.page.currentPage-1}&resname=${requestScope.resname}&status=${requestScope.status}">上一页</a></li>
        </c:if>
        <c:forEach begin="${requestScope.page.SPage}" end="${requestScope.page.EPage}" var="i">
            <li class="page-item ${i==requestScope.page.currentPage?'active':''}"><a class="page-link" href="${pageContext.request.contextPath}/res/doc?cur=${i}&resname=${requestScope.resname}&status=${requestScope.status}">${i}</a></li>
        </c:forEach>
        <c:if test="${requestScope.page.currentPage<requestScope.page.totalPages}">
            <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/res/doc?cur=${requestScope.page.currentPage+1}&resname=${requestScope.resname}&status=${requestScope.status}">下一页</a></li>
            <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/res/doc?cur=${requestScope.page.totalPages}&resname=${requestScope.resname}&status=${requestScope.status}">尾页</a></li>
        </c:if>
...
<div class="toast-container position-fixed bottom-0 end-0 p-3">
    <div id="liveToast" class="toast" role="alert" aria-live="assertive" aria-atomic="true">
        <div class="toast-header">
            ...
            <strong class="me-auto">文档资源管理</strong>
            <small>刚刚</small>
...
<script>
    ...
    $(document).on("click","button[id^=ban]",function (){
        ...
        window.location.href="${pageContext.request.contextPath}/res/doc?act=ban&id="+id;
    })
    $(document).on("click","button[id^=recover]",function (){
        let id = $(this).prop("name");
        window.location.href="${pageContext.request.contextPath}/res/doc?act=recover&id="+id;
    })
    $(document).on("click","button[id^=pass]",function (){
        let id = $(this).prop("name");
        window.location.href="${pageContext.request.contextPath}/res/doc?act=pass&id="+id;
    })
